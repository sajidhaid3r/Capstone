...
The project is split into five single-responsibility modules instead of one
monolithic script, so each concern can be read, tested, and changed
independently:

| Module | Responsibility |
|---|---|
| `gemini_client.py` | Everything Gemini: `SYSTEM_PROMPT` persona, `INGREDIENT_LIST_SCHEMA` / `VOICE_PARSE_SCHEMA` / `RECIPE_SCHEMA`, `.env` loading (`load_env_file`), key resolution (`resolve_api_key`, `configure_gemini`), the fallback engine (`run_gemini_operation`), and the three call types (`detect_ingredients_from_image`, `parse_voice_input`, `generate_recipe`). UI-agnostic except for the `st.session_state`/`st.secrets`/`st.toast` calls the fallback engine needs. |
| `data_pipeline.py` | Pandas data pipeline — `ingredients_to_dataframe`, `dataframe_to_ingredients`, `macros_to_dataframe`, `history_to_dataframe`. Every ingredient/macro/history transform passes through here with defensive try/except fallbacks. |
| `ui_theme.py` | `inject_custom_css()` — the glassmorphism styling block, isolated so visual iteration never touches app logic. |
| `ui_components.py` | Presentation layer — `render_recipe_flashcards`, `render_budget_progress`, `render_history_trend`. Imports DataFrame helpers from `data_pipeline.py`; has no direct Gemini dependency. |
| `app.py` | Orchestration only: page config, session state init, sidebar (targets form + multi-chat session switcher), the three input tabs (camera/voice/manual), the ingredient-confirmation form, and output rendering. Imports from all four modules above; contains no Gemini calls or DataFrame logic of its own. |

**Dependency direction:** `app.py` → `{gemini_client, data_pipeline, ui_theme, ui_components}`,
and `ui_components.py` → `data_pipeline.py`. No circular imports; `gemini_client.py`
and `data_pipeline.py` have no dependency on the UI layer, so either could be
unit-tested or reused (e.g. in a CLI or API service) without pulling in Streamlit
rendering code.
...
**Design:** `run_gemini_operation(api_call_fn)` (in `gemini_client.py`) takes
any Gemini call as a function argument, then:
...
3. On a retriable error (`429`, `503`, `500`, `502`, `timeout`, `deadline`,
   ...
   shows a `st.toast` reading **"TRY AGAIN"**, waits a short, deliberate
   `RETRY_BACKOFF_SECONDS` pause (0.4s — long enough for a transient 503 to
   clear, short enough that a 6-key rotation still resolves in ~2s worst
   case) before retrying
...
This pause is a deliberate, bounded design choice rather than an accidental
blocking call: Streamlit already re-executes the whole script synchronously
on every rerun, so a short sleep inside a single API-call retry does not
change the app's concurrency model — it only adds a small, capped delay to
the one request that hit a retriable error.
...
Every function in `data_pipeline.py` applies the same defensive pattern to
every DataFrame transform.
...
- `requirements.txt`: `streamlit>=1.40.0` (required for `st.audio_input`),
  `google-genai>=1.19.0`, `pandas>=2.2.0` — exactly three dependencies, no
  OS-level dependencies, and no `python-dotenv` since `.env` loading is
  handled by a ~15-line in-house function (`gemini_client.load_env_file`)
